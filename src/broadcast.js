...
export async function seedBroadcastDeliveries(campaignId, env, limit = 250) {
  const campaign = await env.IVAI_DB
    .prepare("SELECT id, status FROM broadcast_campaigns WHERE id = ? LIMIT 1")
    .bind(campaignId)
    .first();
  if (!campaign || !["confirmed", "queued"].includes(campaign.status)) return { seeded: 0, status: campaign?.status || "missing" };
  ...
  for (const delivery of rows) {
    try {
      ...
      await env.IVAI_DB
        ...
        .bind(Number(delivery.attempts || 0) + 1, delivery.id)
        .run();
      sent += 1;
    } catch (error) {
      ...
      await env.IVAI_DB
        ...
        .bind(blocked ? "blocked" : "failed", Number(delivery.attempts || 0) + 1, message, delivery.id)
        .run();
    }
  }
