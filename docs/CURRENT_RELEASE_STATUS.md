...
This document is the concise operational companion to the project’s historical research, audit, debugging, and launch-readiness notes. Release v3.3.40 makes `/new` a complete fresh-Agent action: it clears the active chat Session and atomically restores Auto mode, automatic free model routing, and Memory off. The interface language remains a user-facing display preference. It retains the v3.3.39 separate Mode confirmation, the Auto free-route reset, and v3.3.37 idempotent handling for Telegram no-op message edits. Historical documents remain useful evidence for why a control exists; they do not supersede the current code or this release summary.
