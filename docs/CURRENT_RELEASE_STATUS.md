...
This document is the concise operational companion to the project’s historical research, audit, debugging, and launch-readiness notes. Release v3.3.39 sends a separate confirmation message before it redraws the Menu after a Mode choice, ensuring a user sees the change even when an edited Menu is out of view. Auto remains a complete free-route reset: selecting Menu Auto or `/auto` atomically clears any pinned provider/model, so the next turn can use automatic free routing again. It retains the v3.3.37 idempotent handling for Telegram no-op message edits. Historical documents remain useful evidence for why a control exists; they do not supersede the current code or this release summary.
...
| Response modes | Selecting Auto, Fast, Deep, or Code in the Menu persists the mode, sends a standalone localized confirmation reply to the current Menu, and only then redraws the Menu. Selecting **Auto** also atomically clears `selected_model`, restores automatic free-route choice, and visibly confirms `Model route: Auto`; Fast, Deep and Code preserve an intentional selected model. The `/auto` command follows the same Auto-route rule. |
