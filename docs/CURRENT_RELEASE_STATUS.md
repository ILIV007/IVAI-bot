...
This document is the concise operational companion to the project’s historical research, audit, debugging, and launch-readiness notes. Release v3.3.38 defines Auto as a complete free-route reset: selecting Menu Auto or `/auto` atomically clears any pinned provider/model, so the next turn can use automatic free routing again. It retains the v3.3.37 idempotent handling for Telegram no-op message edits. Historical documents remain useful evidence for why a control exists; they do not supersede the current code or this release summary.
...
| Response modes | Selecting Auto, Fast, Deep, or Code in the Menu persists the mode and immediately confirms the newly active mode before redrawing the Menu. Selecting **Auto** also atomically clears `selected_model`, restores automatic free-route choice, and visibly confirms `Model route: Auto`; Fast, Deep and Code preserve an intentional selected model. The `/auto` command follows the same rule. |
