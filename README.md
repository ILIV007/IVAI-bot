# IVAI Bot v3.3.33 — Free-Tier, Secure Telegram AI Assistant
...
> **IVAI** is an English-first Telegram AI assistant built for Cloudflare Workers. Persian is an optional second language. The v3.3.33 release preserves the familiar v3.2 workflow—modes, free-model catalog, model lock, memory controls, provider fallback, and space-themed interaction—while sending a uniformly selected sticker from the official IVAI pack before every `/start` welcome. The sticker step uses Web Crypto rejection sampling over a static, bot-owned file-ID allow-list, makes no AI call, and never blocks the welcome if Telegram temporarily rejects it.
>
...
| [Changelog](CHANGELOG.md) | Release-level changes through v3.3.33. |
