# IVAI Bot v3.3.39 — Free-Tier, Secure Telegram AI Assistant
...
> **IVAI** is an English-first Telegram AI assistant built for Cloudflare Workers. Persian is an optional second language. In v3.3.39, changing Auto/Fast/Deep/Code from the Menu first sends a separate, visible confirmation message and then redraws the Menu, so the update cannot be missed in edited chat history. Auto still clears any pinned provider/model in the same preference update, restoring automatic free-route selection. `/start` stickers remain uniformly selected, model-picker callbacks use stable model-ID tokens, UI metadata is independent from prompt language, and Telegram's unchanged-message edit response is treated as an idempotent no-op instead of a retry-triggering webhook failure. These controls remain free-only, make no extra AI call, and retain graceful Telegram fallbacks. See the [current release status](docs/CURRENT_RELEASE_STATUS.md) for the canonical operational snapshot.
>
...
| Resource | Purpose |
|---|---|
| [Current release status](docs/CURRENT_RELEASE_STATUS.md) | Canonical v3.3.39 operating snapshot, validation gates, user-visible behavior, and how to read historical records. |
...
| [Changelog](CHANGELOG.md) | Release-level changes through v3.3.39. |
