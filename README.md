# IVAI Bot v3.3.40 — Free-Tier, Secure Telegram AI Assistant
...
> **IVAI** is an English-first Telegram AI assistant built for Cloudflare Workers. Persian is an optional second language. In v3.3.40, `/new` resets both the active conversation and the Agent defaults in one predictable action: **Auto** mode, automatic free model routing, and Memory off. The interface language remains the user's display preference. Changing Auto/Fast/Deep/Code from the Menu sends a separate visible confirmation before redraw, while Auto clears any pinned provider/model in the same preference update. `/start` stickers remain uniformly selected, model-picker callbacks use stable model-ID tokens, UI metadata is independent from prompt language, and Telegram's unchanged-message edit response is treated as an idempotent no-op instead of a retry-triggering webhook failure. These controls remain free-only, make no extra AI call, and retain graceful Telegram fallbacks. See the [current release status](docs/CURRENT_RELEASE_STATUS.md) for the canonical operational snapshot.
>
...
| Resource | Purpose |
|---|---|
| [Current release status](docs/CURRENT_RELEASE_STATUS.md) | Canonical v3.3.40 operating snapshot, validation gates, user-visible behavior, and how to read historical records. |
...
| [Public launch marketing checklist (FA)](docs/TELEGRAM_PUBLIC_LAUNCH_MARKETING_CHECKLIST_FA.md) | Permission-first, manual introduction checklist for the official channel and third-party Telegram groups; includes anti-spam safeguards, 24-hour monitoring and approved copy templates. |
...
| [Changelog](CHANGELOG.md) | Release-level changes through v3.3.40. |
